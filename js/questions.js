(function() {
  ...
  function addQuestion() {
	  ...
	  var answer = {
		  ...
		  skip_conditions: document.getElementById('skip_conditions').value,
		  validation: document.getElementById('validation').value,
	  };
	
	  db.put(answer, function callback(err,result){
		  if (!err){
			  console.log('Successfully saved the question!');
			  showDocs();
			  clearForm();
		  } else {
		  	console.log('Error encountered saving the question!');
		  }
	  });
  }
  ...
  function showDocumentSummary(row){
	...
	rowDisplay.innerHTML = row._id+ " : "+row.question;
	return rowDisplay;
  }
  ...
  function addEventListeners() {
	save_question.addEventListener( 'click', addQuestion);
  }
